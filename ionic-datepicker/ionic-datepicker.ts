import * as moment from 'moment';

import { Component, OnInit, Input, EventEmitter, Output, Inject, ElementRef, forwardRef } from '@angular/core';
import { DatePickerOptions } from "./ionic-datepicker-options";
import { DateModel } from "./ionic-datepicker-model";
import { CalendarDate } from "./ionic-datepicker-calendar-date";
import { NG_VALUE_ACCESSOR } from "@angular/forms";

const Moment: any = (<any>moment).default || moment;

export const CALENDAR_VALUE_ACCESSOR: any = {
  provide: NG_VALUE_ACCESSOR,
  useExisting: forwardRef(() => IonicDatepicker),
  multi: true
};

@Component({
  selector: 'ionic-datepicker',
  templateUrl: 'ionic-datepicker.html',
  providers: [CALENDAR_VALUE_ACCESSOR]
})
export class IonicDatepicker implements OnInit {
  @Input() options: DatePickerOptions;
  @Input() inputEvents: EventEmitter<{ type: string, data: string | DateModel | Array<Date> }>;
  @Output() outputEvents: EventEmitter<{ type: string, data: string | DateModel | Date }>;

  date: DateModel;

  opened: boolean;
  currentDate: moment.Moment;
  days: CalendarDate[];
  events: Array<moment.Moment>;
  years: number[];
  yearPicker: boolean;
  canPrev: boolean;
  canNext: boolean;

  minDate: moment.Moment | any;
  maxDate: moment.Moment | any;

  private onTouchedCallback: () => void = () => { };
  private onChangeCallback: (_: any) => void = () => { };

  constructor(@Inject(ElementRef) public el: ElementRef) {
    this.opened = false;
    this.currentDate = Moment();
    this.options = this.options || {};
    this.days = [];
    this.years = [];
    this.events = [];
    this.canPrev = true;
    this.canNext = true;
    this.date = new DateModel({
      day: null,
      month: null,
      year: null,
      formatted: null,
      momentObj: null
    });

    this.generateYears();

    this.outputEvents = new EventEmitter<{ type: string, data: string | DateModel | Date }>();

    if (!this.inputEvents) {
      return;
    }

    this.inputEvents.subscribe((event: { type: string, data: string | DateModel | Array<Date> }) => {
      if (event.type === 'setDate') {
        this.value = event.data as DateModel;
      } else if (event.type === 'default') {
        if (event.data === 'open') {
          this.open();
        } else if (event.data === 'close') {
          this.close();
        }
      }
    });
  }

  get value(): DateModel {
    return this.date;
  }

  set value(date: DateModel) {
    if (!date) return;

    this.date = date;
    this.onChangeCallback(date);
  }

  updateEvents() {
    if (this.options.events instanceof Array) {
      this.options.events.forEach((event) => {
        this.events.push(Moment(event));
      });
    }
  }

  ngOnInit(): void {
    this.options = new DatePickerOptions(this.options);

    if (this.options.initialDate instanceof Date) {
      this.currentDate = Moment(this.options.initialDate);
      this.selectDate(null, this.currentDate);
    }

    if (this.options.minDate instanceof Date) {
      this.minDate = Moment(this.options.minDate);
    } else {
      this.minDate = null;
    }

    if (this.options.maxDate instanceof Date) {
      this.maxDate = Moment(this.options.maxDate);
    } else {
      this.maxDate = null;
    }

    this.updateEvents();

    this.generateCalendar();
    this.outputEvents.emit({ type: 'default', data: 'init' });

    if (this.inputEvents) {
      this.inputEvents.subscribe((e: any) => {
        if (e.type === 'events') {
          if (e.data instanceof Array && e.data.length) {
            this.options.events = e.data;
            this.updateEvents();
            this.generateCalendar();
          }
        }
        if (e.type === 'action') {
          if (e.data === 'toggle') {
            this.toggle();
          }
          if (e.data === 'close') {
            this.close();
          }
          if (e.data === 'open') {
            this.open();
          }
        }

        if (e.type === 'setDate') {
          if (!(e.data instanceof Date)) {
            throw new Error(`Input data must be an instance of Date!`);
          }
          let date: moment.Moment = Moment(e.data);
          if (!date) {
            throw new Error(`Invalid date: ${e.data}`);
          }
          this.value = {
            day: date.format('DD'),
            month: date.format('MM'),
            year: date.format('YYYY'),
            formatted: date.format(this.options.format),
            momentObj: date
          };
        }
      });
    }
  }

  generateYears() {
    let date: moment.Moment = this.options.minDate || Moment().year(Moment().year() - 40);
    let toDate: moment.Moment = this.options.maxDate || Moment().year(Moment().year() + 40);
    let years = toDate.year() - date.year();

    for (let i = 0; i < years; i++) {
      this.years.push(date.year());
      date.add(1, 'year');
    }
  }

  selectDate(e: MouseEvent, date: moment.Moment) {
    if (e) { e.preventDefault(); }

    setTimeout(() => {
      this.value = {
        day: date.format('DD'),
        month: date.format('MM'),
        year: date.format('YYYY'),
        formatted: date.format(this.options.format),
        momentObj: date
      };
      this.generateCalendar();
    });

    if (this.options.autoApply === true && this.opened === true) {
      this.opened = false;
    }
  }

  generateCalendar() {
    let date: moment.Moment = Moment(this.currentDate);
    let month = date.month();
    let year = date.year();
    let n = 1;
    let firstWeekDay = (this.options.firstWeekdaySunday) ? date.date(2).day() : date.date(1).day();

    if (firstWeekDay !== 1) {
      n -= (firstWeekDay + 6) % 7;
    }

    this.days = [];
    let selectedDate: moment.Moment = this.date.momentObj;
    let max = 34 + n;
    let maxMonth = date.endOf('month').date();

    for (let i = n; i <= max; i += 1) {
      let currentDate: moment.Moment;

      if (i <= 0) {
        currentDate = Moment(`01.${month + 1}.${year}`, 'DD.MM.YYYY');
        currentDate.subtract(-i + 1, 'days');
      } else if (i > maxMonth) {
        currentDate = Moment(`${maxMonth}.${month + 1}.${year}`, 'DD.MM.YYYY');
        currentDate.add(i - maxMonth, 'days');
      } else {
        currentDate = Moment(`${i}.${month + 1}.${year}`, 'DD.MM.YYYY');
      }

      let today: boolean = (Moment().isSame(currentDate, 'day') && Moment().isSame(currentDate, 'month')) ? true : false;
      let selected: boolean = (selectedDate && selectedDate.isSame(currentDate, 'day')) ? true : false;
      let enabled: boolean = true;

      if (this.minDate !== null) {
        if (this.maxDate !== null) {
          enabled = currentDate.isBetween(this.minDate, this.maxDate, 'day', '[]') ? true : false;
        } else {
          enabled = currentDate.isBefore(this.minDate, 'day') ? false : true;
        }
      } else {
        if (this.maxDate !== null) {
          enabled = currentDate.isAfter(this.maxDate, 'day') ? false : true;
        }
      }

      let day: CalendarDate = {
        day: currentDate.date(),
        month: i > 0 ? month : null,
        year: i > 0 ? year : null,
        enabled: enabled,
        today: i > 0 && today,
        selected: i > 0 && selected,
        momentObj: currentDate,
        prevMonth: i <= 0,
        nextMonth: i > maxMonth,
        hasEvent: false
      };

      this.days.push(day);
    }

    this.events.forEach((date) => {
      if (date.diff(this.currentDate, 'month') === 0) {
        this.days.forEach((day) => {
          if (day.momentObj.isSame(date)) day.hasEvent = true;
        });
      }
    });

    this.canPrev = Moment(this.currentDate).subtract(1, 'month').startOf('month').isSameOrAfter(this.minDate);
    this.canNext = Moment(this.currentDate).add(1, 'month').startOf('month').isSameOrBefore(this.maxDate);
  }

  selectYear(e: MouseEvent, year: number) {
    e.preventDefault();

    setTimeout(() => {
      let date: moment.Moment = this.currentDate.year(year);
      this.value = {
        day: date.format('DD'),
        month: date.format('MM'),
        year: date.format('YYYY'),
        formatted: date.format(this.options.format),
        momentObj: date
      };
      this.yearPicker = false;
      this.generateCalendar();
    });
  }

  writeValue(date: DateModel) {
    if (!date) { return; }
    this.date = date;
  }

  registerOnChange(fn: any) {
    this.onChangeCallback = fn;
  }

  registerOnTouched(fn: any) {
    this.onTouchedCallback = fn;
  }

  prevMonth() {
    this.currentDate = this.currentDate.subtract(1, 'month');
    this.generateCalendar();
    this.outputEvents.emit({ type: 'month', data: this.currentDate.toDate() });
  }

  nextMonth() {
    this.currentDate = this.currentDate.add(1, 'month');
    this.generateCalendar();
    this.outputEvents.emit({ type: 'month', data: this.currentDate.toDate() });
  }

  today() {
    this.currentDate = Moment();
    this.selectDate(null, this.currentDate);
  }

  toggle() {
    this.opened = !this.opened;
    if (this.opened) {
      this.onOpen();
    }
  }

  open() {
    this.opened = true;
    this.onOpen();
  }

  close() {
    this.opened = false;
  }

  onOpen() {
    this.yearPicker = false;
  }

  openYearPicker() {
    setTimeout(() => this.yearPicker = true);
  }
}
